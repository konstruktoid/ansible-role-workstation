...
**Applies to:** `{tasks,defaults,meta,molecule}/**/*.{yml,yaml}`

Apply these rules to Ansible YAML changes in this role. They extend the repository-wide instructions
...
- New or changed pinned tool releases (currently `workstation_uv_release`,
  `workstation_opencode_release`, and `workstation_gh_release`) must update the matching entry under
  `shasums:` in `defaults/main.yml`, for every architecture, and keep checksum verification in the
  download task. opencode publishes no checksums for its CLI archives, so those values are computed
  from the downloaded assets; record them anyway, since they pin the reviewed artifact.
...
- `molecule/default/verify.yml` runs as a separate `ansible-playbook` invocation and therefore cannot
  ...
  `workstation_opencode_release`, `workstation_gh_release`, `workstation_packages`,
  `workstation_docker_packages`, and the hardening values below them) means updating that block in the
  same change, or the assertions silently check a stale expectation.
...
- The Docker Engine apt repository and GPG key setup (`tasks/docker.yml`), including the keyring path
  under `/etc/apt/keyrings`, and the removal of the system-wide GitHub CLI apt package, repository, and
  keyring in `tasks/github_cli.yml`, which is the only elevating block in that file.
...
- Favor patterns aligned with the upstream security guidance of each installed tool (Docker's official
  apt-repository install method, the release archives the GitHub CLI and opencode publish on GitHub)
  over ad hoc or curl-pipe-to-shell alternatives.
